...
- `soundtrack.js` / `music-synth.js` / `music.js`: four original scores, a small generated sample bank, and continuous music playback. A shuffle bag avoids immediate repeats; compatible openings and endings crossfade over twelve seconds. The audio clock drives a bounded lookahead scheduler independently of physics. Music has separate saved controls, softens in menus, and fades/suspends on focus loss or mute. Its separate context lets muting music leave sound effects available. See `SOUNDTRACK.md` for arrangement and export details.
