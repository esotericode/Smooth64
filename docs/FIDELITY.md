...
|---|---|
| Collision traversal | libsm64 tests its loaded triangle groups instead of the N64 spatial-cell linked lists. Smooth64 puts the static triangles back in a grid, but each cell keeps load order and covers a wall check's full reach, so every query finds exactly what libsm64's full scan did; the original's misses of walls near a cell edge (a partition bug) do not come back. Floor/ceiling selection order, overlapping surfaces, seams, and tie cases can still differ from the N64's linked lists. |
...
| Port behavior | libsm64 already contains host adaptations and `AVOID_UB` changes. The pinned C matches upstream byte for byte except four recorded patches (`vendor/manifest.json`): 64-bit products in the floor/ceiling tests and in triangle normals (in 32 bits a triangle over ~46,000 units across could vanish, or flip from floor to ceiling), the raised no-floor/no-ceiling heights, and the grid over the static surfaces. On every world they change nothing: random play from every checkpoint gives identical state bytes before and after each change (67,500 ticks for the limits, 118,800 ticks in four worlds and 50,400 among random clutter for the grid). |
