...
export function poseFor(s,name='ACT_IDLE') {
  const timing=animationTiming[s.animation]||[0,1];
  ...
  const p={root:[0,80,0],rotation:[0,0,0],scale:[1,1,1],
    ...
    footPitch:[0,0],handScale:[1,1],eyes:1,accent:0};
  ...
  if(name.includes('LEDGE')) {
    ...
    if(name.includes('LAND')) {
      p.root[1]=mix(38,80,smooth(progress));p.scale=[1.14,.72,1.14];
      p.hands=[[-53,-15,24],[53,-15,24]];p.feet=[[-27,-26,25],[27,-26,25]];
    } else {
    ...
    if(kick) {
      ...
      p.feet=[[ -24,-59,-10],[22,-22,62+35*extend]];
    ...
    p.feet=[[-24,mix(-46,-67,recover),14],[24,mix(-46,-67,recover),14]];
  } else if(air) {
    ...
    const stride=Math.sin(cycle),lift=Math.cos(cycle);
    ...
    p.root[1]+=Math.sin(s.tick*.065)*1.2;
    p.hands[0][1]+=Math.sin(s.tick*.065)*1.2;p.hands[1][1]=p.hands[0][1];
    if(name.includes('SLEEP')){p.eyes=.12;p.rotation[0]=.18;}
  }
  return p;
}
